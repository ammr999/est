...
import '../establishmentsdata/academiesdata.dart';
import '../../constants.dart';




class AcademiesListMenu extends StatefulWidget {
  @override
  _SportCourtsMenuState createState() => _SportCourtsMenuState();
}

class _SportCourtsMenuState extends State<AcademiesListMenu> {
  ...
  void _showGymDetails(BuildContext context, Map<String, dynamic> gym) {
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text('بيانات الأكاديميات الخاصة'),
          content: Column(
  ...
  Widget build(BuildContext context) {
    List<Map<String, dynamic>> filteredData = courts.where((gym) {
      return gym['EstablishmentName'].toString().contains(filterText) ||
          gym['EstablishmentAtivities'].toString().contains(filterText);
    }).toList();
                ...
                return GestureDetector(
                  ...
                  child: Card(
                    ...
                    child: Column(
                      ...
                      children: [
                        ListTile(
                          title: Text('EstablishmentName: ${filteredData[index]['EstablishmentName']}'),
                        ),
                        if (selectedGymIndex == index) Divider(),
                        if (selectedGymIndex == index)
                          for (var entry in filteredData[index].entries)
                            if (entry.key != 'EstablishmentName')
                              Padding(
                                padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
                                child: Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
